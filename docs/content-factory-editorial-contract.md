...
Full and compact article run responses include a bounded `hostedQualityIssues` list for claim-specific hosted quality errors. Each item has a claim ID, a short reason, and an optional component ID. Image findings use the captured image index only when the reviewed image count and signed component inventory agree; verified-resource findings can point to the resource CTA. Findings with no safe component match remain in the list without a page anchor. The projection requires the exact available render, preview URL, preview attempt generation, and a review time after preview startup. It supplies no section removal authority. The persisted quality report and approval gate remain authoritative.

...
- Direct replay of fresh source reports exposed another total-selection defect: a substring match could select an individual contractor expense account as total expenses. Financial labels now match exactly in priority order, and signed category adjustments are retained. All eight privately captured monthly reports now satisfy Revenue minus costs equals reported net result using the release parser. Four real February threads with subsequent replies also pass period-membership replay. This is source replay, not a completed live generation canary.

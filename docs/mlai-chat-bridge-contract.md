...
When no explicit hint is waiting, one of the three priority turns checks a
source-only conversation with activity in the last seven days and no confirmed
read snapshot. Other priority turns continue to refresh known unreads; the
fourth turn remains oldest-first for historical rooms. Source activity only
changes polling order and never assigns an unread badge or expands consent.

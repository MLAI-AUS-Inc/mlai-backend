# Proposed advice-reward CI test migrations

This is a proposal only; no approval or migration execution is implied by this document.

The backend PR workflow creates disposable SQLite and PostgreSQL test databases and runs migration regression tests. It applies 343 existing migrations, including the 309 in the [focused local wallet-test plan](advice-rewards-test-migration-plan.md), then rolls back and replays selected existing migrations inside those test databases.

Approval of this plan would cover applying, rolling back and replaying the exact existing migrations listed below only in newly created disposable local or GitHub Actions test databases for this advice-reward change. It would not authorize production migrations, production data, new migration files or real wallet credits. The focused local runner continues to enforce its narrower 309-migration closure.

This inventory was read from Django's migration graph without opening a database. It describes the existing migration files at backend commit `65e8eae8a32310e51e7d5bdf893d849c7ab8350e`. If that migration inventory changes, revalidate it before execution.

Workflow: [Deploy to Digital Ocean](../.github/workflows/deploy.yml), jobs `checks`, `postgres-search` and `migration-tests`. Do not open the backend PR until this test-only migration scope is approved, because PR creation starts those jobs automatically.

Full existing CI migration inventory (343 migrations):
- `admin.0001_initial`
- `admin.0002_logentry_remove_auto_add`
- `admin.0003_logentry_add_action_flag_choices`
- `auth.0001_initial`
- `auth.0002_alter_permission_name_max_length`
- `auth.0003_alter_user_email_max_length`
- `auth.0004_alter_user_username_opts`
- `auth.0005_alter_user_last_login_null`
- `auth.0006_require_contenttypes_0002`
- `auth.0007_alter_validators_add_error_messages`
- `auth.0008_alter_user_username_max_length`
- `auth.0009_alter_user_last_name_max_length`
- `auth.0010_alter_group_name_max_length`
- `auth.0011_update_proxy_permissions`
- `auth.0012_alter_user_first_name_max_length`
- `community_chat.0001_initial`
- `community_chat.0002_device_auth`
- `community_chat.0003_device_installation_metadata`
- `community_chat.0004_bootstrap_token_origin`
- `community_chat.0005_email_code_challenges`
- `community_chat.0006_account_sessions`
- `community_chat.0007_active_installation_constraint`
- `community_chat.0008_token_usage`
- `community_chat.0009_volunteer`
- `community_chat.0010_moderator`
- `content_analytics.0001_initial`
- `content_analytics.0002_article_analytics_location_history`
- `content_analytics.0003_articleperformancereport_and_more`
- `content_factory.0001_split_content_factory_apps`
- `content_factory.0002_move_legacy_contenttypes`
- `content_factory.0003_website_baseline_snapshot`
- `content_factory.0004_vibe_marketing_component_comment`
- `content_factory.0005_vibe_marketing_component_comment_anchor`
- `content_factory.0006_topic_feedback`
- `content_factory.0007_backfill_topic_coverage_memory`
- `content_factory.0008_researchedkeyword_difficulty_source`
- `content_factory.0009_researchedkeyword_topic_picker_metrics`
- `content_factory.0010_backfill_review_draft_delivery_mode`
- `content_factory.0011_vibe_marketing_component_comment_context`
- `content_factory.0012_research_automations`
- `content_factory.0013_writtenarticle_wa_org_created_idx`
- `content_factory.0014_contentfactorycallbackevent`
- `content_factory.0014_writtenarticle_publish_status`
- `content_factory.0015_merge_20260610_0823`
- `content_factory.0016_writtenarticle_source_run_id`
- `content_factory.0017_backfill_writtenarticle_source_run_id`
- `content_factory.0018_notification_channel_verification`
- `content_factory.0019_notification_delivery_per_channel_keys`
- `content_factory.0020_organizationcontentconfig_article_system_setup_cache_and_more`
- `content_factory.0021_organizationcontentconfig_auto_publish_requires_review`
- `content_factory.0022_writtenarticle_content_path_and_more`
- `content_factory.0023_design_snapshot`
- `content_factory.0024_generatedcomponent_import_statement_metadata`
- `content_factory.0025_orgconfig_use_component_library`
- `content_factory.0026_organizationcontentconfig_authors_and_more`
- `content_factory.0027_healing_record_archived_state`
- `content_factory.0028_organizationcontentconfig_scan_artifact_cache`
- `content_factory.0029_notificationchannel_delivery_enabled`
- `content_factory.0030_callback_event_lease`
- `content_factory.0030_learning_entries_and_healing_framework`
- `content_factory.0031_merge_callback_lease_learning_entries`
- `content_factory.0032_writtenarticle_analytics_identity`
- `content_factory.0033_activate_account_email_notification_channels`
- `content_factory.0034_keywordvelocity_trend_provenance`
- `content_factory.0035_notificationdelivery_performance_report_and_more`
- `content_factory.0036_researched_keyword_ai_search_metrics`
- `content_factory.0037_content_islands`
- `content_factory.0038_delete_seo_topicmap_researchsession`
- `contenttypes.0001_initial`
- `contenttypes.0002_remove_content_type_name`
- `core.0001_initial`
- `core.0002_hackathon`
- `core.0003_user_avatar_url`
- `core.0004_user_about_user_phone`
- `core.0005_user_first_name_user_last_name`
- `core.0006_split_full_name`
- `core.0007_remove_user_full_name`
- `core.0008_user_personas`
- `core.0009_globalsettings`
- `core.0010_user_slack_id_alter_globalsettings_is_obscured`
- `core.0011_organization_organizationcontentconfig`
- `core.0012_organizationcontentconfig_scan_summary_and_more`
- `core.0013_organizationcontentconfig_resource_prompt`
- `core.0014_add_component_library_storage`
- `core.0015_add_company_context`
- `core.0016_organization_competitors`
- `core.0017_add_seed_keywords_to_organization`
- `core.0018_add_content_factory_job`
- `core.0019_contentfactoryjob_request_meta_and_more`
- `core.0020_add_installed_packages_to_orgconfig`
- `core.0021_add_org_github_credentials`
- `core.0022_add_articles_scaffolded`
- `core.0023_add_domain_to_paa_and_aisaturation`
- `core.0024_add_thread_context_to_contentfactoryjob`
- `core.0025_backfill_domain_on_paa_and_aisaturation`
- `core.0026_add_scaffold_preview_url`
- `core.0027_seed_default_hackathons`
- `core.0028_update_hospital_hackathon_metadata`
- `core.0029_add_research_memory_fields`
- `core.0030_add_content_factory_runs`
- `core.0031_add_article_system_to_org_config`
- `core.0032_update_content_factory_status_choices`
- `core.0033_add_content_factory_progress_tracking`
- `core.0034_add_content_factory_billing_fields`
- `core.0035_add_content_factory_progress_message_tracking`
- `core.0035_add_publish_targets_to_org_config`
- `core.0035_rename_cf_run_workflow_status_idx_content_fac_workflo_10aee3_idx_and_more`
- `core.0036_merge_content_factory_progress_and_publish_targets`
- `core.0037_alter_organizationcontentconfig_article_system`
- `core.0038_merge_content_factory_schema_branches`
- `core.0039_scheduled_discovery_dispatch`
- `core.0040_alter_contentfactoryjob_status`
- `core.0041_organizationcontentconfig_connected_slack_user_id`
- `core.0042_add_article_delivery_mode_to_org_config`
- `core.0043_add_content_factory_healing_records`
- `core.0044_alter_contentfactoryjob_status_blocked`
- `core.0045_org_config_repo_execution_contract_and_job_pr_statuses`
- `core.0046_alter_contentfactoryrun_status_and_more`
- `core.0047_organizationcontentconfig_daily_discovery_fields`
- `core.0049_viberaisingpendingsignup`
- `core.0050_delete_viberaisingpendingsignup`
- `core.0051_split_content_factory_apps`
- `core.0052_remove_user_role_has_team`
- `core.0053_user_updated_at`
- `core.0054_rebrand_hospital_as_healthhack`
- `core.0055_passwordresetchallenge_user_auth_version_and_more`
- `core.0056_user_community_chat_profile_id`
- `core.0057_passwordresetemaildelivery`
- `core.0058_drop_orphan_tables_from_removed_apps`
- `core.0058_slackfounderaccountlink_slackfounderlinkrequest`
- `core.0059_purge_stale_content_types`
- `core.0059_slackfounderaccountlink_distinct_users`
- `core.0060_slackfounderlinkrequest_consumed_by_user`
- `core.0061_clear_synthetic_web_slack_ids`
- `core.0062_slackfounderlinkrequest_created_index`
- `core.0063_canonicalize_legacy_content_factory_actor_ids`
- `core.0064_guard_legacy_actor_migration_history`
- `core.0065_recheck_legacy_actor_migration_attestation`
- `core.0066_guard_orphaned_actor_migration_history`
- `core.0067_merge_0058_drop_orphan_tables_from_removed_apps_0066_guard_orphaned_actor_migration_history`
- `core.0068_merge_0059_purge_stale_content_types_0067_merge_0058_drop_orphan_tables_from_removed_apps_0066_guard_orphaned_actor_migration_history`
- `esafety.0001_initial`
- `esafety.0002_submission`
- `esafety.0003_announcement`
- `esafety.0004_team_avatar_url`
- `esafety.0005_alter_announcement_body`
- `esafety.0006_submission_coarse_score_submission_fine_score_and_more`
- `esafety.0007_submission_logs`
- `esafety.0008_alter_submission_logs`
- `founder_tools.0001_move_vibe_raising_models`
- `founder_tools.0002_company_organization`
- `founder_tools.0003_backfill_company_organizations`
- `founder_tools.0004_move_legacy_contenttypes`
- `founder_tools.0005_viberaisingcompany_location`
- `founder_tools.0006_repair_company_organization_column`
- `founder_tools.0007_viberaisingcompany_avatar_url`
- `founder_tools.0008_viberaisingcompany_abr_verified_at_and_more`
- `founder_tools.0008_viberaisingcompany_uniq_profile_domain`
- `founder_tools.0009_merge_20260701_0348`
- `founder_tools.0010_company_default_audience_visibility`
- `generic_hackathons.0001_initial`
- `generic_hackathons.0002_wattthehacksettings`
- `generic_hackathons.0003_generichackathonteam_leader`
- `generic_hackathons.0004_generichackathonjoinrequest_and_more`
- `generic_hackathons.0005_generichackathonteam_eval_token`
- `generic_hackathons.0005_watt_session_cache_table`
- `generic_hackathons.0006_merge_20260604_0238`
- `generic_hackathons.0007_alter_generichackathonteam_eval_team_uuid_and_more`
- `hospital.0001_initial`
- `hospital.0002_alter_team_members`
- `hospital.0003_alter_submission_user`
- `hospital.0004_announcement`
- `hospital.0005_medhack_from_roo`
- `hospital.0006_team_avatar_url`
- `hospital.0007_add_feedback_to_submission`
- `hospital.0008_simcasewinner_simdiagnosisguess_and_more`
- `hospital.0009_sim_observability_and_prizes`
- `hospital.0010_sim_ai_npc_tools`
- `hospital.0011_sim_prize_registration`
- `hospital.0012_sim_turn_idempotency`
- `hospital.0013_rebrand_announcements_for_healthhack`
- `hospital.0014_announcement_slack_provenance`
- `hospital.0015_hospital_competition_round`
- `hospital.0016_announcement_round_and_activate_healthhack`
- `hospital.0017_delete_medhack_game_and_prediction`
- `integrations.0001_initial`
- `integrations.0002_userintegration`
- `integrations.0003_add_github_repo_field`
- `integrations.0004_userintegration_last_scanned_at_and_more`
- `integrations.0005_add_github_refresh_token_fields`
- `integrations.0006_gmailthreadartifact_startupprofile_gmailsynccursor_and_more`
- `integrations.0007_startupprofile_stage`
- `integrations.0008_alter_gmailattachmentartifact_content_disposition_and_more`
- `integrations.0009_communitybridgechannel_communitybridgereceipt_and_more`
- `integrations.0010_external_service_connection`
- `integrations.0011_split_content_factory_apps`
- `integrations.0012_financial_account_external_record`
- `integrations.0013_xero_financial_record_types`
- `integrations.0014_startup_metric_source_provenance`
- `integrations.0015_slack_artifacts`
- `integrations.0016_split_startup_updates_app`
- `integrations.0017_alter_external_service_provider_choices`
- `integrations.0018_google_analytics_provider_choice`
- `integrations.0019_luma_provider_choice`
- `integrations.0020_externalserviceconnection_uniq_user_org_provider_account`
- `integrations.0021_googleconnection_organization_and_more`
- `integrations.0022_githubinstallation`
- `integrations.0023_githubinstallation_liveness_checked_at`
- `integrations.0024_stripe_xero_reconciliation`
- `integrations.0025_reconciliation_context_suggestions`
- `integrations.0026_xero_statement_reconciliation`
- `integrations.0027_linear_issue_creation_receipt`
- `integrations.0028_xero_statement_posting`
- `integrations.0029_humanitix_reconciliation_foundation`
- `integrations.0030_xero_statement_scan_and_confidence`
- `integrations.0031_reconciliation_rules_and_decisions`
- `integrations.0032_reconciliation_decision_approval_types`
- `integrations.0033_xero_statement_reconciliation_outcomes`
- `integrations.0034_reconciliation_learning_decisions`
- `integrations.0035_event_source_provenance`
- `integrations.0035_generic_community_bridge_destination`
- `integrations.0036_community_bridge_identity_links`
- `integrations.0036_xero_statement_scan_capture_metadata`
- `integrations.0037_community_bridge_reaction_delivery_types`
- `integrations.0037_reconciliation_profitability_policy`
- `integrations.0038_account_backed_bridge_identity`
- `integrations.0039_merge_20260802_1530`
- `integrations.0040_mandatory_reconciliation_tracking`
- `integrations.0041_linear_project_sizing_runs`
- `integrations.0042_linear_meeting_action_batches`
- `integrations.0043_slack_dm_mirroring`
- `integrations.0044_slack_dm_group_history_backfill`
- `integrations.0045_communitybridgedelivery_parent_dependency`
- `integrations.0046_communitybridgedeletionrequest`
- `jobs.0001_initial`
- `jobs.0002_jobrun_execution_config`
- `mlai_studio.0001_initial`
- `org_memory.0001_service_identity`
- `org_memory.0002_organization_authorization`
- `org_memory.0003_source_control_plane`
- `org_memory.0004_evidence_kernel`
- `org_memory.0005_memoryconnectionconfiguration_next_scheduled_sync_at_and_more`
- `org_memory.0006_memorychunkembedding_memorychunk_search_vector_and_more`
- `org_memory.0007_drivedocumentartifact_drivedocumentartifactversion_and_more`
- `org_memory.0008_drivemeeting_and_more`
- `org_memory.0009_alter_driveinventorymanifest_counts_and_more`
- `org_memory.0010_memoryclaim_memoryextractionrun_memoryevidence_and_more`
- `org_memory.0011_memoryentity_merged_at_memoryentity_merged_into_and_more`
- `org_memory.0012_memoryquerylog_memoryfeedback_and_more`
- `org_memory.0013_provider_event_receipts`
- `org_memory.0014_notion_artifacts`
- `org_memory.0015_gmail_scoped_artifacts`
- `org_memory.0016_structured_aggregate_artifact`
- `org_memory.0017_memorydailyreconciliationreport_and_more`
- `org_memory.0018_review_summaries_digests`
- `org_memory.0019_memorypublication_publicknowledgeitem_and_more`
- `org_memory.0020_agentactionproposal_agentactionevent_and_more`
- `org_memory.0021_memory_selector_shadow`
- `org_memory.0022_memory_pilot_query_audit`
- `org_memory.0023_memory_pilot_deployment`
- `org_memory.0024_alter_memoryoutboxevent_event_type`
- `org_memory.0025_delete_selector_shadow`
- `organizations.0001_split_content_factory_apps`
- `organizations.0002_organization_company_linkedin_url`
- `roo.0001_initial`
- `roo.0002_points_system_v2`
- `roo.0003_remove_pointsadmin_name`
- `roo.0004_seed_rewards`
- `roo.0005_tasktemplate`
- `roo.0006_rewardscatalog_stock_remaining`
- `roo.0007_populate_pilot_rules`
- `roo.0008_populate_comprehensive_rate_card`
- `roo.0009_add_weekly_allowance`
- `roo.0010_channelfirstpost`
- `roo.0011_questprogress`
- `roo.0012_medhack_game_models`
- `roo.0013_move_medhack_to_hospital`
- `roo.0014_add_content_factory_ledger_source`
- `roo.0015_pointsrequest`
- `roo.0016_rename_roo_pointsr_status_8f1eab_idx_roo_pointsr_status_1880e1_idx_and_more`
- `roo.0017_points_health_indexes`
- `roo.0018_ensure_coworking_unique_active_booking`
- `roo.0019_structured_task_engine`
- `roo.0020_taskassignment_claimed_points_snapshot`
- `roo.0021_alter_taskactivity_event_type`
- `roo.0022_add_partner_points_admin_role`
- `roo.0023_pointsaccount_earned_balance_and_more`
- `roo.0024_pointspurchase`
- `roo.0025_update_rewards_pricing_and_hide_coffee`
- `roo.0026_coworking_day_cost_eight`
- `roo.0027_alter_ledger_source`
- `roo.0028_pointspurchase_checkout_sessions`
- `roo.0029_boostpostadmission_and_more`
- `roo.0030_microroo_and_coding_billing`
- `roo.0031_meeting_room_booking`
- `roo.0032_small_and_big_meeting_rooms`
- `roo.0033_meeting_room_purchased_cost_microroo`
- `roo.0034_coworkingbookingoperation`
- `roo.0034_officemanagerday_coworkingbooking_booking_source_and_more`
- `roo.0035_coworkingbookingoperation_subjects`
- `roo.0035_protect_office_manager_assignment_day`
- `roo.0036_office_manager_attempts_and_provenance`
- `roo.0036_sanitize_coworking_operation_receipts`
- `roo.0037_quarantine_legacy_office_manager_provenance`
- `roo.0038_office_manager_claim_generation`
- `roo.0039_supersede_reopened_office_manager_attempts`
- `roo.0040_merge_coworking_operations_office_manager`
- `sessions.0001_initial`
- `startup_updates.0001_split_startup_updates_app`
- `startup_updates.0002_move_legacy_contenttypes`
- `startup_updates.0003_gmailattachmentartifact_last_error`
- `startup_updates.0003_slackthreadartifact_relevance_fields`
- `startup_updates.0004_linear_artifacts`
- `startup_updates.0005_merge_linear_gmail_attachment`
- `startup_updates.0006_repair_slack_thread_relevance_columns`
- `startup_updates.0007_startupprofile_organization_kind`
- `startup_updates.0008_alter_gmailmessageartifact_relevance_label_and_more`
- `startup_updates.0009_startup_data_deletion_request`
- `startup_updates.0010_startup_manual_document`
- `startup_updates.0011_startupprofile_company_detail_fields`
- `startup_updates.0012_googleanalyticspropertyselection_and_more`
- `startup_updates.0013_luma_event_selection`
- `startup_updates.0014_monthlyupdatedraft_ready_at`
- `startup_updates.0015_backfill_monthly_update_ready_at`
- `startup_updates.0016_userstartupbinding_monthly_updates_enabled`
- `startup_updates.0017_monthly_update_visibility_publish_state`
- `startup_updates.0017_userstartupbinding_coworking_discount_eligible`
- `startup_updates.0018_monthlyupdatereminderdelivery`
- `startup_updates.0019_merge_20260723_0335`
- `startup_updates.0020_luma_event_attendance_counts`
- `startup_updates.0021_linear_project_member_artifact`
- `startup_updates.0022_reporting_evidence_revisions`
- `vibe_raising.0001_initial`
- `vibe_raising.0002_move_founder_tools_models`
- `victor_ai.0001_initial`
- `victor_ai.0002_victorapplication_revenue_last_3_months_and_more`
- `victor_ai.0003_victorapplication_linkedin`
- `victor_ai.0004_victor_roo_access`
- `workflow_runs.0001_split_content_factory_apps`
- `workflow_runs.0002_contentfactoryrun_cf_run_domain_wf_updated_idx`
- `workflow_runs.0003_contentfactoryrun_last_event_emitted_at`
- `workflow_runs.0004_contentfactoryrun_organization_and_more`
- `workflow_runs.0005_contentfactoryrun_reconciled_at`

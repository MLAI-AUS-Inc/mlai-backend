...
from integrations.models import FinancialConnection, FinancialConnectionStatus
...
class FinancialStatusView(APIView):
    def get(self, request):
        organization = _get_authorized_organization(request, request.query_params.get("domain"))
        return Response(get_financial_status(organization=organization), status=status.HTTP_200_OK)


class FinancialSyncView(APIView):
    def post(self, request):
        organization = _get_authorized_organization(request, request.data.get("domain"))
        connection_ids = list(request.data.get("connection_ids") or [])
        try:
...
class FinancialConnectionDetailView(APIView):
    def delete(self, request, connection_id: int):
        connection = get_object_or_404(
            FinancialConnection.objects.select_related("organization", "user"),
            id=connection_id,
        )
        if not _user_can_access_organization(request.user, connection.organization):
            return Response({"error": "Not found."}, status=status.HTTP_404_NOT_FOUND)

        connection.status = FinancialConnectionStatus.DISCONNECTED
        connection.access_token = ""
        connection.refresh_token = ""
        connection.last_error = ""
        connection.save(update_fields=["status", "access_token", "refresh_token", "last_error", "updated_at"])
        return Response({"status": "disconnected", "connection_id": connection.id}, status=status.HTTP_200_OK)
...
class FinancialRunCreateView(APIView):
    ...
    def post(self, request):
        ...
        organization = get_object_or_404(Organization, domain=normalize_domain(data["domain"]))
        user_id = data.get("user_id") or request.data.get("user_id")
        user = get_object_or_404(User, pk=user_id) if user_id else None
        if user is None:
            connection = organization.financial_connections.order_by("-updated_at").first()
...
class FinancialRunSnapshotsView(APIView):
    ...
    def get(self, request, run_id: str):
        ...
        snapshots = organization.monthly_revenue_snapshots.order_by("-month", "currency")[:24]
        return Response(
            {
                "run": _serialize_financial_run(run),
...
class StripeFinancialWebhookView(APIView):
    ...
    def post(self, request):
        ...
        connections = list(
            FinancialConnection.objects.select_related("organization", "user").filter(
                provider="stripe",
...
def _get_authorized_organization(request, raw_domain) -> Organization:
    ...
    if not _user_can_access_organization(request.user, organization):
        raise serializers.ValidationError({"domain": "No startup binding found for this user and domain."})
    return organization


def _user_can_access_organization(user, organization: Organization) -> bool:
    ...
    if organization.user_startup_bindings.filter(user=user).exists():
        return True
    return organization.financial_connections.filter(user=user).exists()

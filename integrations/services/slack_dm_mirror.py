...
class SlackDmMirrorRateLimited(SlackDmMirrorError):
    """A workspace/method is waiting for Slack's Retry-After deadline."""
...
def _refresh_slack_grant_token_if_due(
    ...
    with transaction.atomic():
        ...
        if locked_expiry is None or locked_expiry > timezone.now() + timedelta(
            ...
            return SlackDmMirrorGrant.objects.select_related("connection").get(
                pk=locked_grant.pk
            )
        ...
        access_token = str(
            authed_user.get("access_token") or token_data.get("access_token") or ""
        ).strip()
        replacement_refresh_token = str(
            authed_user.get("refresh_token") or token_data.get("refresh_token") or ""
        ).strip()
...
def _lock_slack_grant_api_authority(
    ...
    *,
    ...
    authority_current = _slack_grant_api_authority_is_current(
        ...
        current_slack_oauth_generation(authority.user_id) == authority.oauth_generation
    )
...
def _call_slack_with_grant_authority(
    ...
    **kwargs,
    ...
    cooldown_key = f"slack-chat-rate:{authority.workspace_id}:{method}"
    if float(cache.get(cooldown_key) or 0) > time.time():
        raise SlackDmMirrorRateLimited(
            "Slack is limiting requests; import will resume automatically."
        )
    with transaction.atomic():
        ...
        try:
            ...
            if retry_after:
                cache.set(cooldown_key, time.time() + retry_after, timeout=retry_after)
            raise
...
def open_slack_dm(
    ...
    *,
    ...
    shadow_count = len(
        {
            key
            for slack_id, key in (conversation.participant_identity_map or {}).items()
            if slack_id != grant.slack_user_id
        }
    )
    ...
    return {
        ...
        "needs_reauthorization": bool(
            ...
            and (
                ...
                or (
                    grant is not None
                    and grant.consent_version not in PRIVATE_CHANNEL_CONSENTS
                )
        ...
        "history_scope": "all" if history_days == 0 else "recent",
        "consent_version": (
        ...
        "privacy": {
            ...
            "history_is_bounded": history_days > 0,
...
def activate_connection(
    ...
    *,
    ...
    if history_days == 0 and not include_private_channels:
        raise SlackDmMirrorError(
            "Choose all available history from MLAI Chat to consent."
        )
    consent_version = (
        (ALL_HISTORY_CONSENT if history_days == 0 else PRIVATE_CHANNEL_CONSENT)
        if include_private_channels
        and PRIVATE_CHANNEL_SCOPES.issubset(set(connection.scopes or []))
        else SlackDmMirrorGrant.CONSENT_VERSION
    )
    requested_history_days = _bounded_history_days(
        history_days, allow_all=consent_version == ALL_HISTORY_CONSENT
    )
    ...
    if (
        ...
        and existing_grant.connection_id == connection.pk
        and existing_grant.status == SlackDmMirrorGrantStatus.ACTIVE
        and existing_grant.revoked_at is None
    ):
        # The owner is expanding the same live connection. Keep the existing
        # private registrations; reconnect must not perform hundreds of DELETEs
        ...
        with transaction.atomic():
            ...
            current.last_discovery_at = None
            current.last_error = ""
            current.save(
                update_fields=(
                    "consent_version",
    ...
    with transaction.atomic():
        ...
        if grant is None:
            ...
            if _registration_cleanup_pending_locked(grant.pk):
                ...
                grant.history_days = requested_history_days
...
def backfill_grant(
    ...
    *,
    ...
    # Only an explicit new history_days=0 selection widens consent. Keep the
    # historical full_history/backfill_all alias bounded for installed clients.
    if history_days == 0 and not full_history:
        if not private_channels_enabled(grant):
            raise SlackDmMirrorError(
                "Reconnect Slack before importing all available history."
            )
        grant.consent_version = ALL_HISTORY_CONSENT
    selected_days = history_days if history_days is not None else grant.history_days
    grant.history_days = _bounded_history_days(
        selected_days,
        allow_all=grant.consent_version == ALL_HISTORY_CONSENT and not full_history,
    )
    if grant.history_days > 0 and grant.consent_version == ALL_HISTORY_CONSENT:
...
def _clear_slack_connection_locked(
    ...
    # Keep a content-free type fence so late group-channel events cannot be
    # mistaken for MPIM events after private-channel consent is revoked.
    type_fences = catalog_tombstones(connection.provider_metadata)
...
def _retire_ineligible_conversation(
    ...
    *,
    ...
    with transaction.atomic():
        grant = (
            SlackDmMirrorGrant.objects.select_for_update().filter(pk=grant_id).first()
        )
        if grant is None:
            return False
        conversation_query = (
...
def _stage_unknown_slack_event(
    ...
    *,
    ...
    for candidate in candidates:
        if private_channel and not private_channels_enabled(candidate):
            continue
        ...
        with transaction.atomic():
            ...
            if private_channel and not private_channels_enabled(grant):
                continue
            sync_cursor = dict(connection.sync_cursor or {})
...
def _complete_inactive_conversation_history(
    ...
    *,
    ...
    with transaction.atomic():
        ...
        defer_quiet_history_jobs(conversation)
...
def discover_conversations(
    ...
    while True:
        ...
        for raw in raw_channels:
            ...
            if (
                bool(raw.get("is_archived"))
                and grant.consent_version != ALL_HISTORY_CONSENT
            ):
                _retire_ineligible_from_slack_response(
                    authority,
                    channel_id,
    ...
    with transaction.atomic():
        ...
        if locked_grant.pk == grant.pk:
            ...
            locked_grant.last_discovery_at = None if needs_followup else timezone.now()
...
def _discover_conversation(
    ...
    *,
    ...
    kind = raw_conversation_kind(raw)
    if kind is None or (
        kind == "private_channel" and not private_channels_enabled(grant)
    ):
        return None
    required_scopes = DIRECT_DM_SCOPES | (
        PRIVATE_CHANNEL_SCOPES
        if kind == "private_channel"
        else GROUP_DM_SCOPES if kind == "mpim" else set()
    )
    ...
    if not recent_activity:
        ...
        # Membership revocations above still apply. A confirmed quiet mirror
        # must not create another relay room merely because devices changed or
        # OAuth was renewed. Recent activity can provision it on a later pass.
...
def _enqueue_normalized_slack_event_locked(
    ...
    if (
        ...
        target_message_id = str(metadata.get("target_source_message_id") or "").strip()
        original = (
...
def ingest_slack_dm_event(payload: dict[str, Any]) -> dict[str, Any] | None:
    ...
    is_group_dm_event = str(event.get("channel_type") or "").strip().lower() in {
        "mpim",
        "group",
    }
...
def ingest_mlai_dm_event(payload: dict[str, Any]) -> dict[str, Any] | None:
    ...
    conversation = (
        SlackDmMirrorConversation.objects.select_related("grant")
        .filter(
    ...
    with transaction.atomic():
        ...
        if conversation is None:
            return {"status": "ignored"}
        conversation.grant = grant
        if conversation_metadata(conversation).get("source_archived"):
            return {"status": "rejected", "error": "slack_conversation_archived"}
...
def _backfill_delivery_is_outside_history_window(
    ...
    *,
    ...
    if delivery.source_platform != CommunityBridgePlatform.SLACK:
        return False
    # Delayed callbacks and retries obey the same source window as history.
    # Deletions must still remove already-imported content, even after it ages.
    if delivery.operation == CommunityBridgeDeliveryType.DELETE:
        return False
    now = now or timezone.now()
    if history_days is None:
        history_days = delivery.conversation.grant.history_days
    history_days = _bounded_history_days(
        history_days,
        allow_all=getattr(delivery.conversation.grant, "consent_version", "")
        == ALL_HISTORY_CONSENT,
    )
    if history_days == 0:
        return False
    cutoff = int(now.timestamp()) - history_days * 86_400
    metadata = delivery.metadata or {}
    for value in (metadata.get("target_source_message_id"), delivery.source_message_id):
        try:
            return _slack_ts_sort_key(str(value or ""))[0] < cutoff
        except SlackDmMirrorError:
            continue
...
def _claim_ready_private_delivery_batch(*, limit: int) -> list[SlackDmMirrorDelivery]:
    ...
    with transaction.atomic():
        ...
        if (
            ...
            or SlackDmMirrorDelivery.objects.filter(
                conversation=conversation,
                status=CommunityBridgeDeliveryStatus.PROCESSING,
            ).exists()
        ):
...
def _record_private_delivery_failure(
    ...
    with transaction.atomic():
        ...
        if (
            dependency_pending
            and delivery.source_platform == CommunityBridgePlatform.SLACK
        ):
    ...
    for grant in grants:
        try:
            ...
            if (
                ...
                grant.last_discovery_at = (
                    None
                    if isinstance(exc, SlackDmMirrorRateLimited)
                    or _slack_retry_after_seconds(exc)
                    else timezone.now()
                )
...
def _prepare_owner_conversation_locked(
    ...
    *,
    ...
    is_private_channel = conversation_kind(conversation) == "private_channel"
    ...
    if grant.slack_user_id not in participant_ids or (
        not is_private_channel and not 2 <= len(participant_ids) <= 9
    ):
    ...
    shared_import_identity = (
        is_private_channel or conversation_kind(conversation) == "mpim"
    )
    # Slack provenance carries each source author separately from the transport
    # key. One import key leaves room for the owner's desktop, phone and browser
    # even when Slack's group DM already has nine people.
    owner_capacity = 8 if shared_import_identity else 9 - len(counterpart_ids)
    ...
    identity_map.update(
        {
            participant_id: _shadow_pubkey(
                conversation,
                "private-channel-import" if shared_import_identity else participant_id,
            )
    ...
    pubkeys = sorted(
        {
            *identity_map.values(),
            *owner_device_pubkeys,
            *(
                [_shadow_pubkey(conversation, "private-channel-import")]
...
def _mark_conversation_history_due(
    ...
    *,
    ...
    if (
        ...
        and not reset_deliveries
        and conversation.history_backfilled_at is None
    ):
        return
    now = timezone.now()
    _clear_history_scan_states([conversation.pk])
    ...
    if reconcile_current_state:
        _mark_history_reconciliation_candidates_locked(conversation, recent_only=True)
    if reset_deliveries:
        SlackDmMirrorDelivery.objects.filter(
            conversation=conversation,
            source_platform=CommunityBridgePlatform.SLACK,
...
def _mark_history_reconciliation_candidates_locked(
    ...
    *,
    ...
    reconcile_oldest = _reconciliation_oldest(conversation, recent_only=recent_only)
    rows = list(
    ...
    for row in rows:
        ...
        try:
            ...
            if metadata != original_metadata:
                ...
                changed.append(row)
            continue
        if reconcile_oldest and (
            seconds < int(reconcile_oldest)
            or not _message_is_within_reconciliation_scope(row, reconcile_oldest)
        ):
            if metadata != original_metadata:
                row.metadata = metadata
                changed.append(row)
    ...
    # Persist even an empty scan's boundary; inferring it only from imported
    # message rows would accidentally restart an all-history scan on every open.
    _ensure_history_state(
        conversation,
        source_message_id=HISTORY_RECONCILIATION_STATE_ID,
        metadata={
            "history_scan_state": "reconciliation",
            HISTORY_RECONCILE_EPOCH_KEY: reconcile_epoch,
            HISTORY_RECONCILE_OLDEST_KEY: reconcile_oldest,
        },
    )


def _message_is_within_reconciliation_scope(
    row: SlackDmMirrorDelivery, oldest: str
) -> bool:
    """Only infer deletion when this history scan can observe the whole thread."""
    if not oldest:
        return True
    metadata = row.metadata or {}
    if row.source_platform == CommunityBridgePlatform.BUZZ and metadata.get(
        "source_parent_message_id"
    ):
        # This is a relay event ID, not a Slack timestamp. Until its thread is
        # explicitly fetched, a main-history page cannot prove the reply absent.
        return False
    parent_ts = str(
        metadata.get("thread_ts") or metadata.get("original_thread_ts") or ""
    ).strip()
    source_ts = (
        row.source_message_id
        if row.source_platform == CommunityBridgePlatform.SLACK
        else str(metadata.get("slack_ts") or "")
    )
    if not parent_ts or parent_ts == source_ts:
        return True
    try:
        parent_seconds, _ = _slack_ts_sort_key(parent_ts)
    except SlackDmMirrorError:
        return False
    # conversations.history does not enumerate replies on roots before oldest.
    # Signed delete callbacks remain authoritative for these excluded replies.
    return parent_seconds >= int(oldest)
...
def _history_reconciliation_boundary_locked(
    ...
    marker = _history_state(conversation, HISTORY_RECONCILIATION_STATE_ID)
    if marker is not None:
        metadata = marker.metadata or {}
        boundaries.add(
            (
                str(metadata.get(HISTORY_RECONCILE_EPOCH_KEY) or ""),
                str(metadata.get(HISTORY_RECONCILE_OLDEST_KEY) or ""),
            )
        )
    boundaries.discard(("", ""))
    if len(boundaries) > 1:
    ...
    for _ in range(scan_limit):
        ...
        try:
            ...
            if _is_slack_auth_error(exc):
                user = (
                    get_user_model()
                    .objects.filter(pk=conversation.grant.user_id)
                    .first()
                )
                if user is not None:
                    revoke_user_grant(user)
...
def _history_required_scopes(
    slack_conversation_id: str, *, kind: str | None = None
) -> set[str]:
    if kind == "private_channel":
        return DIRECT_DM_SCOPES | PRIVATE_CHANNEL_SCOPES
    if kind == "mpim" or str(slack_conversation_id or "").startswith("G"):
        return DIRECT_DM_SCOPES | GROUP_DM_SCOPES
    return set(DIRECT_DM_SCOPES)


def _require_private_channel_consent(conversation: SlackDmMirrorConversation) -> None:
    if conversation_kind(
        conversation
    ) == "private_channel" and not private_channels_enabled(conversation.grant):
        raise SlackDmMirrorAuthorizationError(
            "Reconnect Slack to authorize private-channel import."
        )
...
def _history_scan_authority_from_state(
    ...
    if (
        ...
        or (
            authority.history_days == 0
            and authority.oldest
            and not authority.oldest.isdigit()
        )
    ):
...
def _assert_history_scan_authority_locked(
    ...
    current_registration = bool(
        ...
        and _registration_generation(registration) == expected.registration_generation
...
def _enqueue_history_page(
    ...
    authority = _capture_slack_grant_api_authority(grant)
    required_scopes = _history_required_scopes(
        conversation.slack_conversation_id, kind=conversation_kind(conversation)
    )
    (
        scan_authority,
        slack_conversation_id,
    ...
    from integrations.services.message_sync.history_policy import history_page_limit
...
def _persist_history_page_locked(
    ...
    if (
        (
            ...
            or (response.get("response_metadata") or {}).get("next_cursor")
        )
        and boundary_ids
        and conversation.oldest_synced_ts
    ):
        if min(map(_slack_ts_sort_key, boundary_ids)) >= _slack_ts_sort_key(
            conversation.oldest_synced_ts
        ):
            raise SlackDmMirrorError("Slack history pagination made no progress.")
    current_days = _grant_history_days(conversation.grant)
    current_cutoff = int(time.time()) - current_days * 86_400 if current_days else 0
    # A long-running scan's frozen oldest bound never broadens today's consent
    # window; provider responses are checked again before storing any bodies.
    source_cutoff = max(current_cutoff, int(Decimal(scan_authority.oldest or "0")))
    history = []
    participant_ids = set(conversation.participant_slack_ids or [])
    for message in raw_messages:
        try:
            if _slack_ts_sort_key(str(message.get("ts") or ""))[0] < source_cutoff:
                continue
        except SlackDmMirrorError:
            continue
        if not _history_message_author_allowed(conversation, message):
            continue
        message = _normalize_history_author(conversation, message)
        message_id = str(message.get("ts") or "").strip()
        author_id = str(message.get("user") or "").strip()
        if (
            not message_id
            or not author_id
            or (
                author_id not in participant_ids
                and not _all_history_group_import(conversation)
            )
        ):
            continue
        _slack_ts_sort_key(message_id)
    ...
    for message in history:
        ...
        if (
            ...
            and scan_authority.history_days > 0
            and scan_authority.oldest
        ):
            parent_seconds, _ = _slack_ts_sort_key(parent_message_id)
            parent_outside_window = parent_seconds < source_cutoff
    ...
    if boundary_ids and min(_slack_ts_sort_key(value)[0] for value in boundary_ids) < source_cutoff:
        has_more = False
    update_fields = ["last_error", "updated_at"]
    if boundary_ids:
        ...
        _advance_latest_synced_ts(conversation, newest_in_page)
    ...
    state_metadata["complete"] = not has_more
    state_metadata["source_limited"] = bool(state_metadata.get("source_limited") or response.get("is_limited"))
    state_metadata["observed_messages"] = bool(state_metadata.get("observed_messages") or history)
...
def _enqueue_reply_page(
    ...
    required_scopes = _history_required_scopes(
        slack_conversation_id, kind=conversation_kind(conversation)
    )
    with transaction.atomic():
        conversation, grant = _locked_history_write_context(
            conversation_id, grant_id, authority, required_scopes,
            scan_authority=scan_authority,
        )
        state = SlackDmMirrorDelivery.objects.select_for_update().get(
            pk=state.pk, conversation=conversation,
        )
        metadata = dict(state.metadata or {})
        if str(metadata.get("scan_epoch") or "") != scan_authority.epoch:
            raise SlackDmMirrorAuthorizationError(
                "Slack thread scan was replaced before the request was sent."
            )
        ...
        # Freeze this request's lower bound across cursor pages. A newly begun
        # thread uses today's window; an existing cursor keeps its original
        # source query and every response is still checked against today's scope.
        oldest = str(metadata.get("request_oldest", scan_authority.oldest) or "")
        if not cursor:
            days = _grant_history_days(grant)
            cutoff = max(0, int(time.time()) - days * 86_400) if days else 0
            oldest = str(max(cutoff, int(Decimal(scan_authority.oldest or "0"))))
            metadata["request_oldest"] = oldest
    ...
    if oldest and Decimal(oldest) > 0:
        request_kwargs["oldest"] = oldest
        request_kwargs["inclusive"] = True
    response = _call_slack_with_grant_authority(
        authority,
...
def _all_history_group_import(conversation: SlackDmMirrorConversation) -> bool:
    """Allow source-only historical authors within an explicitly consented group."""
    return getattr(
        conversation.grant, "consent_version", ""
    ) == ALL_HISTORY_CONSENT and conversation_kind(conversation) in {
        "mpim",
        "private_channel",
    }


def _normalize_history_author(
    conversation: SlackDmMirrorConversation, message: dict[str, Any]
) -> dict[str, Any]:
    """Preserve departed-member/bot attribution without granting them relay access."""
    if not _all_history_group_import(conversation):
        return message
    message = dict(message)
    author_id = str(message.get("user") or message.get("bot_id") or "").strip()
    profile = message.get("user_profile") or message.get("bot_profile") or {}
    if not isinstance(profile, dict):
        profile = {}
    icons = profile.get("icons") if isinstance(profile.get("icons"), dict) else {}
    message["user"] = author_id
    message["_mlai_history_author"] = {
        "display_name": str(
...
def _history_delivery_author_pubkey(delivery: SlackDmMirrorDelivery) -> str:
    """Use only an already registered source identity; never add a recipient."""
    ...
    if (
        delivery.source_platform != CommunityBridgePlatform.SLACK
        or not metadata.get("backfill")
        or not _all_history_group_import(conversation)
    ):
        return ""
    # This deterministic key was registered before any page could be scanned.
    # It is shared by group authors and is never one of the owner's devices.
    import_key = _shadow_pubkey(conversation, "private-channel-import")
    return (
        import_key
        if import_key in (conversation.participant_buzz_pubkeys or [])
        else ""
    )
...
def _persist_reply_page_locked(
    ...
    participant_ids = set(conversation.participant_slack_ids or [])
    current_days = _grant_history_days(conversation.grant)
    consent_cutoff = max(0, int(time.time()) - current_days * 86_400) if current_days else 0
    source_cutoff = max(consent_cutoff, int(Decimal(scan_authority.oldest or "0")))
    messages = []
    for message in response.get("messages") or []:
        ...
        if (
            ...
            continue
        # Slack can include the root even when it predates oldest. Context is
        # allowed only inside current consent, never as an exemption for old bodies.
        cutoff = consent_cutoff if message_id == parent_message_id else source_cutoff
        if _slack_ts_sort_key(message_id)[0] < cutoff:
            continue
    ...
    for message in messages:
        ...
        if (current_days and _slack_ts_sort_key(parent_message_id)[0] < consent_cutoff
                and str(message.get("ts") or "") != parent_message_id):
            message["_mlai_original_thread_ts"] = parent_message_id
            message["thread_ts"] = ""
        _enqueue_history_message(
            conversation,
    ...
    has_more = bool(response.get("has_more") or next_cursor)
    if has_more and (
        not next_cursor or next_cursor == str(metadata.get("cursor") or "").strip()
    ):
    ...
    metadata["source_limited"] = bool(metadata.get("source_limited") or response.get("is_limited"))
    metadata["observed_messages"] = bool(metadata.get("observed_messages") or messages)
    state.metadata = metadata
    state.save(update_fields=("metadata", "updated_at"))
    conversation.last_error = ""
    conversation.save(update_fields=("last_error", "updated_at"))
    ...
    if (main_state is not None
            and str((main_state.metadata or {}).get("scan_epoch") or "") == scan_authority.epoch):
        main_metadata = dict(main_state.metadata or {})
        main_metadata["source_limited"] = bool(main_metadata.get("source_limited") or metadata["source_limited"])
        main_metadata["observed_messages"] = bool(main_metadata.get("observed_messages") or metadata["observed_messages"])
        main_state.metadata = main_metadata
        main_state.save(update_fields=("metadata", "updated_at"))
    main_complete = bool(
...
def _enqueue_history_message(
    ...
    *,
    ...
    if _all_history_group_import(conversation):
        metadata["history_author"] = message.get("_mlai_history_author") or {
    ...
    for reaction in message.get("reactions") or []:
        ...
        for reaction_author_id in reaction.get("users") or []:
            ...
            if not reaction_author_id or (
                reaction_author_id not in set(conversation.participant_slack_ids or [])
                and not _all_history_group_import(conversation)
            ):
                continue
            semantic_id = reaction_object_id(
...
def _upsert_history_delivery(
    ...
    *,
    ...
    elif delivery.status == CommunityBridgeDeliveryStatus.COMPLETED and bool(
        (delivery.metadata or {}).get("history_reconcile_candidate")
    ):
    ...
    *,
    ...
    return next(
        (
            ...
            if (
                scan_epoch is None
                or str((state.metadata or {}).get("scan_epoch") or "") == scan_epoch
            )
...
def _finish_history_scan(conversation: SlackDmMirrorConversation) -> None:
    ...
    main_state = SlackDmMirrorDelivery.objects.select_for_update().filter(
        conversation=conversation, source_platform=CommunityBridgePlatform.SLACK,
        source_message_id=HISTORY_MAIN_STATE_ID,
        operation=CommunityBridgeDeliveryType.CREATE,
    ).first()
    metadata = dict(main_state.metadata or {}) if main_state is not None else {}
    source_limited = bool(metadata.get("source_limited"))
    now = timezone.now()
    sync_state = ensure_state(conversation)
    sync_state = BridgeSyncState.objects.select_for_update().get(pk=sync_state.pk)
    ranges = dict(sync_state.verified_ranges or {})
    ranges["archive"] = {
        "classification": "source_limited" if source_limited else (
            "accessible_range" if metadata.get("observed_messages") else "empty_accessible_range"
        ),
        "oldest": str(metadata.get("oldest") or ""),
        "latest": f"{int(now.timestamp())}.999999",
...
def _reconcile_absent_slack_state_locked(
    ...
    state_reconcile_epoch = str(state_metadata.get(HISTORY_RECONCILE_EPOCH_KEY) or "")
    ...
    for candidate in candidates:
        ...
        candidate_reconcile_epoch = str(metadata.get(HISTORY_RECONCILE_EPOCH_KEY) or "")
        ...
        try:
            target_seconds, _ = _slack_ts_sort_key(semantic_target)
        except SlackDmMirrorError:
            continue
        if days and target_seconds < int(now.timestamp()) - days * 86_400:
            # The rolling window deliberately excluded this item. Its absence
            # cannot prove Slack deleted it; only an explicit delete can do so.
            continue
    ...
    with transaction.atomic():
        ...
        for delivery in deliveries:
            ...
            if not _private_delivery_batch_eligible(delivery):
                raise SlackDmMirrorError(
                    "Private delivery batch changed while claimed."
                )
            ...
            profile = (
                ...
                or (delivery.metadata or {}).get("history_author")
                or {}
            )
        ...
        _advance_latest_synced_ts(conversation, latest_timestamp)
        conversation.save(
...
def _assert_slack_conversation_writable(
    conversation: SlackDmMirrorConversation,
) -> None:
    """Archived source history is readable but never a destination for writes."""
    if conversation_metadata(conversation).get("source_archived"):
        raise SlackDmMirrorAuthorizationError(
            "This Slack conversation is archived and read-only."
        )
...
def _deliver_to_slack(delivery: SlackDmMirrorDelivery) -> None:
    conversation = delivery.conversation
    grant = conversation.grant
    _assert_slack_conversation_writable(conversation)
    if (
        conversation.status != SlackDmMirrorConversationStatus.LIVE
        or grant.status != SlackDmMirrorGrantStatus.ACTIVE
        or grant.revoked_at is not None
...
def _deliver_to_mlai(delivery: SlackDmMirrorDelivery) -> None:
    ...
    try:
        ...
        event_timestamp = ""
    _advance_latest_synced_ts(conversation, event_timestamp)
    conversation.save(
        update_fields=("last_synced_at", "latest_synced_ts", "updated_at")
    )
...
def _advance_latest_synced_ts(
    conversation: SlackDmMirrorConversation, timestamp: str
) -> None:
    """Historical page delivery must never move a conversation's recency backward."""
    if timestamp and (
        not conversation.latest_synced_ts
        or _slack_ts_sort_key(timestamp)
        > _slack_ts_sort_key(conversation.latest_synced_ts)
    ):
        conversation.latest_synced_ts = timestamp
...
def _bounded_history_days(value: Any, *, allow_all: bool = False) -> int:
    configured = _history_days()
    try:
        current = int(value)
    except (TypeError, ValueError):
        current = configured
    if current == 0 and allow_all:
        return 0
    if current <= 0:
        return configured
...
def _grant_history_days(grant: SlackDmMirrorGrant) -> int:
    """Honor zero only for grants carrying the explicit all-history consent."""
    return _bounded_history_days(
        grant.history_days,
        allow_all=getattr(grant, "consent_version", "") == ALL_HISTORY_CONSENT,
    )


def _reconciliation_oldest(
    conversation: SlackDmMirrorConversation, *, recent_only: bool
) -> str:
    """Refresh recent source state and any offline gap without rescanning archives."""
    now_seconds = int(time.time())
    days = _grant_history_days(conversation.grant)
    if days:
        return str(max(0, now_seconds - days * 86_400))
    if not recent_only:
        return ""
    oldest = max(0, now_seconds - MAX_HISTORY_DAYS * 86_400)
    if conversation.latest_synced_ts:
        latest, _ = _slack_ts_sort_key(conversation.latest_synced_ts)
        oldest = min(oldest, max(0, latest - 86_400))
    return str(oldest)


def _normalize_grant_history_window_locked(grant: SlackDmMirrorGrant) -> bool:
    """Fence legacy zero-valued scans unless the owner explicitly selected all."""

    bounded_days = _grant_history_days(grant)
...
def _conversation_participant_ids(
    ...
    *,
    ...
    kind = raw_conversation_kind(raw_conversation)
    if kind not in {"mpim", "private_channel"}:
        return []
    ...
    while not checkpoint.get("complete"):
        response = _call_slack_with_grant_authority(
            ...
            required_scopes=DIRECT_DM_SCOPES
            | (
                PRIVATE_CHANNEL_SCOPES if kind == "private_channel" else GROUP_DM_SCOPES
            ),
            channel=channel_id,
            limit=200,
            cursor=cursor,

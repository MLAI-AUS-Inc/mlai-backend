...
class Command(BaseCommand):
    help = (
        "Activate an already staged Admin Roo pilot after live readiness. "
    ...
    def handle(self, *args, **options):
        ...
        if not readiness["ready"]:
            ...
            raise CommandError("Admin Brain pilot activation readiness has blockers.")
        try:
            with transaction.atomic():
                deployment, changed = activate_pilot_deployment(
                    organization=organization,
                    approval_manifest=approval_manifest,

...
RESTRICTED_PATTERNS: tuple[tuple[re.Pattern[str], str | None, str], ...] = (
    ...
    (re.compile(r"\bnot\s+(available\s+)?(?:in|to|for)\s+(apac|asia pacific|anz)\b", re.I), "APAC", "Explicitly excludes APAC/ANZ candidates"),
    (re.compile(r"\bnot\s+(apac|asia pacific|anz)\b", re.I), "APAC", "Explicitly excludes APAC/ANZ candidates"),
    (re.compile(r"\beurope\s+only\b", re.I), "Europe", "Restricted to Europe only"),
    (re.compile(r"\bonly\s+from\s+(the\s+)?(eu|europe|european union)\b", re.I), "Europe", "Restricted to Europe only"),

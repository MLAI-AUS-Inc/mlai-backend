...
class JobFeedback(models.Model):
    ...
    run = models.ForeignKey(JobRun, to_field="run_id", db_column="run_id", related_name="feedback", on_delete=models.CASCADE, blank=True, null=True)
    job = models.ForeignKey(JobListing, related_name="feedback", on_delete=models.SET_NULL, blank=True, null=True)
    feedback_type = models.CharField(max_length=32, choices=FEEDBACK_TYPES, db_index=True)
    rank = models.IntegerField(blank=True, null=True)
    ...
    slack_message_ts = models.CharField(max_length=255, blank=True)
    processed_at = models.DateTimeField(blank=True, null=True)
    created_at = models.DateTimeField(default=timezone.now)


class JobDisqualifierCandidate(models.Model):
    STATUSES = [
        ("review", "Review"),
        ("active", "Active"),
        ("rejected", "Rejected"),
    ...
    category = models.CharField(max_length=64, default="community")
    status = models.CharField(max_length=32, choices=STATUSES, default="review", db_index=True)
    severity = models.CharField(max_length=32, choices=SEVERITIES, default="penalize")

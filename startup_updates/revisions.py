...
def capture_snapshot(organization, month, *, run=None, manual_metrics=None, base_snapshot=None):
    ...
    for key in sorted(keys):
        ...
        candidates = [item for item in candidates if not item.unit or item.unit in {"count", "ratio", "months", "%"} or item.unit == profile.default_currency]
        ...
        metrics.append({
            ...
            "quality": ("partial" if observation.source_provider == "financial" else "source_reported") if observation else "unknown",
    ...
    payload["charts"] = copy.deepcopy(base_snapshot.payload.get("charts", {})) if base_snapshot else build_monthly_financial_snapshot(organization=organization, target_month=month)
    ...
    for point in (payload.get("charts") or {}).get("performance", []):
        if point["month"] == month.isoformat():
            ...
            point["net"] = None  # Do not derive profit from incomplete or mismatched definitions.
    ...
    if run is not None:
        ...
        events = events.filter(id__in=approved)
    payload["events"] = list(events.order_by("id").values(
        "id", "title", "summary", "event_date", "status", "evidence_message_ids", "evidence_attachment_ids",
    ))
    ...
    if base_snapshot is not None:
        ...
        for field in ("events", "startup", "period", "definitions", "config_version", "source_providers"):
